...
def evaluate(config_file="config.yaml"):
    ...
    """
    e = Data()
    e.track_id = torch.from_numpy(evaluated_events["track_id"].to_numpy())  
    e.particle_id = torch.from_numpy(evaluated_events["particle_id"].to_numpy())
    e.n_shared = torch.from_numpy(evaluated_events["n_shared"].to_numpy())
    e.n_reco_hits = torch.from_numpy(evaluated_events["n_reco_hits"].to_numpy())
    e.n_true_hits = torch.from_numpy(evaluated_events["n_true_hits"].to_numpy())
    e.pt = torch.from_numpy(evaluated_events["pt"].to_numpy())
    e.is_matchable = torch.from_numpy(evaluated_events["is_matchable"].to_numpy())
    e.is_reconstructable = torch.from_numpy(evaluated_events["is_reconstructable"].to_numpy())
    e.event_id  = torch.from_numpy(evaluated_events["event_id"].to_numpy())
    e.purity_reco = torch.from_numpy(evaluated_events["purity_reco"].to_numpy())
    e.eff_true = torch.from_numpy(evaluated_events["eff_true"].to_numpy())
    e.is_matched = torch.from_numpy(evaluated_events["is_matched"].to_numpy())
    e.is_reconstructed = torch.from_numpy(evaluated_events["is_reconstructed"].to_numpy())
    # torch.save(e, output_dir + "/01")

    evaluated_events.to_csv(output_dir+'/01.csv', index=False)
