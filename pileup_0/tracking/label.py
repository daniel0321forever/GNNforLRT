...
def label_graph(graph, score_cut=0.8, save_dir="datasets/labeled"):
...
def train(config_file="config.yaml"):
    ...
    for subdir in ["train", "val", "test"]:
        subdir_graphs = os.listdir(os.path.join(track_building_configs["input_dir"], subdir))
        all_graphs += [torch.load(os.path.join(track_building_configs["input_dir"], subdir, graph), map_location="cpu") for graph in subdir_graphs]
