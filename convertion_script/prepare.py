...
def process(evtid, input_dir, output_dir):
    ...
    sp_hits = sp.loc[:, sp.columns!='geometry_id'].merge(meas2hits, how='left', on='measurement_id')
    sp_hits = sp_hits.merge(hits, how='left', on='hit_id')
    ...
    filename = os.path.join(output_dir, str(evtid)) + ".pgy"
    print(filename)
...
if __name__ == "__main__":
    ...
    all_evtids = sorted([int(re.search(r'\d+', os.path.basename(x)).group(0).strip()) for x in all_evts])
    # print(all_evtids)
