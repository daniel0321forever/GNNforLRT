...
def build_edges(query, database, indices, r_max, k_max, return_distances=False):
    ...
    if indices is not None:
        edge_list[0] = indices[edge_list[0]] # turn the source points idx into real index
    edge_list = edge_list[:, edge_list[0] != edge_list[1]] # we want the [sources, targets] tensors, but no self-loop pair
...
def build_edges_frnn(query, database, r_max, k_max):
    ...
    # find the nearest points within radius r_max, and within the amount of k_max
    # the different between query and database (spatial) is that the number of query is limited
    ...
    idxs = idxs.squeeze() # shape = (points, nearest_points)
    print("idx:", idxs.shape) # TODO: del
    ind = torch.Tensor.repeat(torch.arange(idxs.shape[0], device=device), (idxs.shape[1], 1), 1).T # the source points -> (0, .., 99 | * k_max)
    print("ind", ind.shape) # TODO: del
